...
export function errorDetail(error: unknown): string {
  if (error instanceof Error) return truncateDetail(error.message || error.name);
...
function parseHealthEvent(raw: string | null, component: string): HealthEvent | null {
  ...
  try {
    ...
    return {
      component,
      status: parsed.status === 'error' || parsed.status === 'disabled' ? parsed.status : 'ok',
...
export async function readHealthEvents(
  ...
  for (const component of components) {
    events[component] = parseHealthEvent(await db.getSetting(database, healthKey(component)), component);
  }
  return events;
}
