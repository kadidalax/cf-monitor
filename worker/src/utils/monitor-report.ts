import type { MonitorRecord } from '../db/queries';
...
function normalizeGpuList(rawGpu: AnyRecord, rawGpus: unknown): AnyRecord[] {
...
export function normalizeMonitorReport(input: any): AnyRecord {
  const raw = input?.type === 'report' && input.data ? input.data : input;
...
export function toMonitorRecord(client: string, time: string, input: any): MonitorRecord {
