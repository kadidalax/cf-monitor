...
function readToken(value: unknown, fallback: string, errors: string[]): string {
  ...
  const text = value.trim();
  if (text.length < 12 || text.length > 256 || !/^[A-Za-z0-9._:-]+$/.test(text)) {
    errors.push('token 只能包含字母、数字、点、下划线、冒号和连字符，长度 12-256');
    return fallback;
  }
  return text;
}
...
export function validateClientCreateInput(input: unknown): ClientCreateValidationResult {
  ...
  const token = readToken(input.token, crypto.randomUUID(), errors);
