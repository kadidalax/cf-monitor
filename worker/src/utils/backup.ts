import type { Client, PingTask } from '../db/queries';
import { sanitizeSettingsForStorage } from '../settings/schema';
import { validatePingTaskInput } from './ping-task';
...
function backupPasswordError(password: string): string | null {
  if (!password) return '备份密码不能为空';
  if (new TextEncoder().encode(password).byteLength < MIN_BACKUP_PASSWORD_BYTES) {
    return `备份密码至少需要 ${MIN_BACKUP_PASSWORD_BYTES} 字节`;
  }
  return null;
}
...
function validateOfflineNotifications(items: unknown[], errors: string[]): any[] {
  return items.flatMap((item, index) => {
    if (!isPlainObject(item)) {
...
export async function encryptBackup(backup: BackupData, password: string): Promise<BackupEncryptResult> {
  const passwordError = backupPasswordError(password);
...
export async function decryptBackup(input: unknown, password: string): Promise<BackupDecryptResult> {
  ...
  if (!salt || !iv || !ciphertext || iv.length !== 12) {
