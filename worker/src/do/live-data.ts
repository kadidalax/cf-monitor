  ...
  private gpuSnapshotSignature(gpus: unknown): string | null {
    ...
    const devices = gpus
      .map((device: any, index) => {
        const memTotal = Number(device?.mem_total || 0);
        const memBucketSize = Math.max(memTotal * GPU_MEMORY_BUCKET_RATIO, GPU_MEMORY_BUCKET_MIN_UNITS);
        return {
          i: Number(device?.device_index ?? index),
          n: String(device?.device_name || '').slice(0, 64),
          mt: Math.round(memTotal / GPU_MEMORY_BUCKET_MIN_UNITS),
          mu: Math.round(Number(device?.mem_used || 0) / memBucketSize),
          u: Math.round(Number(device?.utilization || 0) / GPU_UTILIZATION_BUCKET_PERCENT),
          t: Math.round(Number(device?.temperature || 0) / GPU_TEMPERATURE_BUCKET_C),
        };
      })
  ...
  private async persistReport(clientId: string, report: any, nowMs: number, force = false): Promise<boolean> {
    ...
    try {
      ...
      if (gpuDecision.persist && gpuDecision.signature) {
        await db.insertGPURecords(this.env.DB, clientId, time, normalizedReport.gpus);
