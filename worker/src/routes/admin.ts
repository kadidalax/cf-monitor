...
import { AGENT_AUTH_CACHE_MS, invalidateAgentClientAuthCache, invalidateAgentPingTaskCache } from './client';
...
const ADMIN_CLIENTS_CACHE_MS = 60_000;
...
const AGENT_AUTH_CACHE_SEC = Math.floor(AGENT_AUTH_CACHE_MS / 1000);
...
async function listAdminClientsCached(database: D1Database, force = false): Promise<db.Client[]> {
  const now = Date.now();
  ...
  adminClientsCache = {
    value: clients,
    expiresAt: now + ADMIN_CLIENTS_CACHE_MS,
  };
  return clients;
}
...
function skippedSchemaProbe(checkedAt: string): HealthEvent {
  return {
    component: 'schema_probe',
    status: 'disabled',
    updated_at: checkedAt,
    detail: 'Schema probe skipped by default; use deep=1 or refresh=1 to run D1 table and column checks',
  };
}

async function buildHealthCheck(c: any, deep: boolean, cacheState: HealthCheckBody['cache']): Promise<{ body: HealthCheckBody; status: 200 | 503 }> {
  const checkedAt = new Date().toISOString();
  ...
  if (d1Probe.status !== 'error') {
    components.schema_probe = deep ? await runSchemaProbe(c.env.DB, checkedAt) : skippedSchemaProbe(checkedAt);
  }
...
adminRoutes.get('/clients', async (c) => {
  ...
  const cacheHit = !refresh && Boolean(adminClientsCache && adminClientsCache.expiresAt > Date.now());
  const clients = await listAdminClientsCached(c.env.DB, refresh);
  c.header('X-CF-Monitor-Admin-Clients-Cache', refresh ? 'refresh' : (cacheHit ? 'hit' : 'miss'));
...
adminRoutes.post('/clients/reorder', async (c) => {
  try {
    ...
    if (updated > 0) invalidateAdminClientsCache();
    invalidatePublicMetadataCache();
    invalidateCapacityEstimateCache();
...
adminRoutes.get('/settings', async (c) => {
  ...
  if (scope) {
    ...
    return c.json(Object.fromEntries(keys.map((key) => [key, settings[key]])));
  }
...
adminRoutes.post('/settings', async (c) => {
  try {
    ...
    await db.setSettings(c.env.DB, changedSettings);
...
adminRoutes.post('/notification/offline/edit', async (c) => {
  try {
    ...
    const changed = await db.setOfflineNotifications(c.env.DB, normalized);
...
adminRoutes.post('/notification/expiry/edit', async (c) => {
  try {
    ...
    const changed = await db.setExpiryNotifications(c.env.DB, normalized);
    return c.json({ success: true, updated: normalized.length, changed, noop: changed === 0 });
  } catch {
    return c.json({ error: '保存失败' }, 500);
  }
...
adminRoutes.get('/health', async (c) => {
  ...
  if (!deep && healthCheckCache && healthCheckCache.expiresAt > Date.now()) {
    c.header('X-CF-Monitor-Health-Cache', 'hit');
  ...
  c.header('X-CF-Monitor-Health-Cache', result.body.cache);
  if (!deep) {
    healthCheckCache = {
      value: result.body,
