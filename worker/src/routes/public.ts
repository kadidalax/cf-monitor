...
function publicCacheRequest(c: any): Request {
  const url = new URL(c.req.url);
  url.searchParams.sort();
  return new Request(url.toString(), { method: 'GET' });
}

function publicJsonResponse(c: any, value: unknown, maxAgeSeconds: number, cacheState: string): Response {
  setPublicCache(c, maxAgeSeconds);
  c.header('X-CF-Monitor-Public-Cache', cacheState);
  return new Response(JSON.stringify(value), {
    headers: {
      'Content-Type': 'application/json; charset=UTF-8',
      'Cache-Control': `public, max-age=${maxAgeSeconds}, s-maxage=${maxAgeSeconds}, stale-while-revalidate=${maxAgeSeconds * 2}`,
      'X-CF-Monitor-Public-Cache': cacheState,
    },
  });
}

function withPublicCacheHeader(c: any, response: Response, maxAgeSeconds: number, cacheState: string): Response {
  setPublicCache(c, maxAgeSeconds);
  c.header('X-CF-Monitor-Public-Cache', cacheState);
  ...
  headers.set('X-CF-Monitor-Public-Cache', cacheState);
...
async function getPublicEdgeCache(c: any, maxAgeSeconds: number): Promise<Response | null> {
...
function putPublicEdgeCache(c: any, response: Response): void {
  ...
  try {
    ...
    // Edge cache is only a quota optimization; correctness comes from D1/DO.
  }
}

async function getPublicHistoryCache(c: any, key: string): Promise<Response | null> {
...
async function getPublicMetadataResponse(c: any): Promise<Response | null> {
...
function setPublicMetadataResponse(c: any, value: unknown): Response {
...
async function preparePublicHistoryRequest(
  ...
  if (cacheIsFresh(cachedVisibility)) {
    ...
    return limited ? { cacheKey, visible: true, response: limited } : { cacheKey, visible: true };
  }

  const snapshot = await getPublicClientsSnapshot(database);
...
publicRoutes.get('/clients', async (c) => {
  ...
  return setPublicMetadataResponse(c, snapshot.clients);
});
...
publicRoutes.get('/public', async (c) => {
  ...
  if (limited) return limited;
  return setPublicMetadataResponse(c, await getPublicSettings(c.env.DB));
});
...
publicRoutes.get('/recent/:uuid', async (c) => {
  ...
  const prepared = await preparePublicHistoryRequest(c, c.env.DB, uuid, 'recent');
...
publicRoutes.get('/records/load', async (c) => {
  ...
  const prepared = await preparePublicHistoryRequest(c, c.env.DB, uuid, 'records-load');
  if (prepared.response) return prepared.response;
  if (!prepared.visible) {
    if (wantsPagedResponse(c)) {
      ...
      return setPublicHistoryCache(c, prepared.cacheKey, emptyPagedResult(page, limit));
    }
  ...
  if (start && end) {
    ...
    if (wantsPagedResponse(c)) {
      ...
      return setPublicHistoryCache(c, prepared.cacheKey, await db.getRecordsByTimeRangePaged(c.env.DB, uuid, start, end, page, limit));
    }

    const limit = readIntParam(limitQuery, 500, 1000);
    return setPublicHistoryCache(c, prepared.cacheKey, await db.getRecordsByTimeRangeLimited(c.env.DB, uuid, start, end, limit));
  }
...
publicRoutes.get('/records/gpu', async (c) => {
  ...
  const prepared = await preparePublicHistoryRequest(c, c.env.DB, uuid, 'records-gpu');
  if (prepared.response) return prepared.response;
  if (!prepared.visible) {
    if (wantsPagedResponse(c)) {
      const page = readIntParam(c.req.query('page'), 1, 100000);
  ...
  if (wantsPagedResponse(c)) {
    const page = readIntParam(c.req.query('page'), 1, 100000);
    return setPublicHistoryCache(c, prepared.cacheKey, await db.getGPURecordsPaged(c.env.DB, uuid, start, end, page, limit));
  }
...
publicRoutes.get('/records/ping', async (c) => {
  ...
  const prepared = await preparePublicHistoryRequest(c, c.env.DB, uuid, 'records-ping');
  if (prepared.response) return prepared.response;
  if (!prepared.visible) {
    if (wantsPagedResponse(c)) {
      const page = readIntParam(c.req.query('page'), 1, 100000);
  ...
  if (wantsPagedResponse(c)) {
    const page = readIntParam(c.req.query('page'), 1, 100000);
    return setPublicHistoryCache(c, prepared.cacheKey, await db.getPingRecordsPaged(c.env.DB, uuid, taskId, page, limit));
  }
...
publicRoutes.get('/records/ping/batch', async (c) => {
  ...
  const prepared = await preparePublicHistoryRequest(c, c.env.DB, uuid, 'records-ping-batch');
  if (prepared.response) return prepared.response;
  if (!prepared.visible) return setPublicHistoryCache(c, prepared.cacheKey, {});
...
publicRoutes.get('/task/ping', async (c) => {
  ...
  return setPublicMetadataResponse(
    c,
    await getPublicPingTasks(c.env.DB, snapshot.publicClientIds, boundedPublicPingIntervalSec(settings)),
  );
});
