    ...
    <Box className="node-table-expanded">
      ...
      <div className="node-table-detail-sections">
        ...
        <DetailSection title="运行状态">
          ...
          <DetailRow label="备注" value={node.public_remark || '-'} />
        </DetailSection>
      </div>

      <div className="node-table-ping-section">
        <MiniPingChart uuid={node.uuid} width="100%" height={210} limit={180} />
      </div>
    </Box>
  );
}
...
export default function NodeTable({ nodes, liveData }: NodeTableProps) {
  ...
  return (
    <Box style={{ overflowX: 'auto', padding: '0 16px' }}>
      <Table.Root variant="surface" size="2">
        <Table.Header>
          <Table.Row>
            <Table.ColumnHeaderCell style={{ width: 40 }} />
            <SortHeader column="name">名称</SortHeader>
            <SortHeader column="os">系统</SortHeader>
            <SortHeader column="status">状态</SortHeader>
            <SortHeader column="cpu">CPU</SortHeader>
            <SortHeader column="ram">内存</SortHeader>
            <SortHeader column="disk">硬盘</SortHeader>
            <SortHeader column="network">网络</SortHeader>
            <SortHeader column="price">价格</SortHeader>
            <SortHeader column="traffic">流量</SortHeader>
          </Table.Row>
        </Table.Header>
              ...
              <React.Fragment key={node.uuid}>
                <Table.Row style={{ cursor: 'pointer' }} onClick={() => toggleExpanded(node.uuid)}>
                  ...
                  <Table.Cell>
                    ...
                    >
                      <Flex align="center" gap="2" style={{ minWidth: 170 }}>
                  ...
                  <Table.Cell>
                    <Flex align="center" gap="2">
                      ...
                      <Text size="2" truncate style={{ maxWidth: 120 }}>{getOSName(node.os)}</Text>
                    </Flex>
                  </Table.Cell>
                  <Table.Cell>
                  ...
                  <Table.Cell>
                    <Text size="2">
                      {node.price ? `${node.currency || '¥'}${node.price}` : '-'}
                    </Text>
                  </Table.Cell>
                  <Table.Cell>
                    <Text size="2" style={{ whiteSpace: 'nowrap' }}>
