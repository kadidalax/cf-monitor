...
export default function AdminPingTasks() {
  ...
  }, [tasks, clients, search, scopeFilter]);

  const serverRows = useMemo(() => {
    return clients.map((client) => ({
      client,
      boundTasks: tasks.filter((task) => task.all_clients || task.clients.includes(client.uuid)),
    }));
  }, [clients, tasks]);
  ...
  return (
    <Flex className="admin-ping-page" direction="column" gap="2">
      ...
      <Card>
        <Flex className="admin-ping-filter-toolbar" gap="3" wrap="wrap" align="center" justify="start">
          <Flex className="admin-ping-filter-controls" gap="2" wrap="wrap" align="center">
          ...
          <Flex className="admin-ping-filter-stats" gap="2" wrap="wrap" align="center">
            <Badge variant="soft" color="blue">当前结果 {filteredTasks.length}</Badge>
            <Badge variant="soft" color="green">全局 {tasks.filter((task) => task.all_clients).length}</Badge>
            <Badge variant="soft" color="amber">定向 {tasks.filter((task) => !task.all_clients).length}</Badge>
          </Flex>
        </Flex>
      </Card>
                  ...
                  <Table.Cell>
                    <Flex gap="1" wrap="wrap">
                      {boundTasks.length > 0 ? boundTasks.map((task) => (
                        <Badge key={task.id} size="1" variant="soft" color={task.all_clients ? 'green' : 'blue'}>
                          {task.name}
                        </Badge>
                      )) : <Text size="1" color="gray">暂无任务</Text>}
                    </Flex>
                  </Table.Cell>
                  <Table.Cell><Text size="1">{boundTasks.length}</Text></Table.Cell>
                </Table.Row>
              ))}
            </Table.Body>
          </Table.Root>
