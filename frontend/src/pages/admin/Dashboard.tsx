...
function SortableRow({ node, selected, onSelect, liveData, onDetail, onEdit, onDelete, onCmd, onRotateToken, dragDisabled }: SortableRowProps) {
  ...
  return (
    <Table.Row ref={setNodeRef} className={`admin-table-row${isDragging ? ' is-dragging' : ''}`} style={rowStyle}>
      ...
      <Table.Cell>
        <Flex align="center" gap="2" style={{ minWidth: 220 }}>
          ...
          <Flex direction="column" style={{ minWidth: 0 }}>
            <Text size="2" weight="bold" style={{ cursor: 'pointer', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 220 }} onClick={() => onDetail(node)} title="查看详情">{node.name || '未命名'}</Text>
      ...
      <Table.Cell>
        <Flex direction="column" style={{ minWidth: 170 }}>
      ...
      <Table.Cell>
        <Flex direction="column" style={{ minWidth: 120 }}>
          <Text size="1" style={{ fontFamily: 'monospace' }}>{node.version || '-'}</Text>
          <Text size="1" color="gray" style={{ maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={[node.os, node.arch].filter(Boolean).join(' / ')}>
            {[node.os, node.arch].filter(Boolean).join(' / ') || '-'}
          </Text>
        </Flex>
      </Table.Cell>
      <Table.Cell><Text size="1" style={{ display: 'block', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={node.group}>{node.group || '-'}</Text></Table.Cell>
      <Table.Cell><Text size="1" color={node.remark ? undefined : 'gray'} style={{ display: 'block', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={node.remark || ''}>{node.remark || '-'}</Text></Table.Cell>
      <Table.Cell>
      ...
      <Dialog.Content style={{ maxWidth: 680 }}>
        ...
        <Flex direction="column" gap="3" style={{ maxHeight: 440, overflow: 'auto', paddingRight: 4 }}>
          ...
          <div className="install-options-grid">
            ...
            <FieldInput label="二进制下载地址" value={installOptions.binaryUrl || ''} onChange={(v) => setOption('binaryUrl', v)} placeholder="为空则使用项目 GitHub release" />
...
export default function AdminDashboard() {
  ...
  return (
    <Flex className="admin-dashboard-page" direction="column" gap="3">
      ...
      <Card className="admin-table-card">
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={filteredUuids} strategy={verticalListSortingStrategy}>
            <Table.Root variant="surface" style={{ minWidth: 1120 }}>
              <Table.Header>
                <Table.Row>
                  <Table.ColumnHeaderCell className="admin-node-select-header" style={{ width: 76 }}>
                  ...
                  <Table.ColumnHeaderCell className="admin-node-name-header">名称</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell>IP 地址</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell>客户端版本</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell>分组</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell>私有备注</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell>账单</Table.ColumnHeaderCell>
                  <Table.ColumnHeaderCell style={{ width: 168 }}>操作</Table.ColumnHeaderCell>
                </Table.Row>
              </Table.Header>
              <Table.Body>
                {filtered.length === 0 ? (
                  <Table.Row><Table.Cell colSpan={8}><Text align="center" color="gray" style={{ display: 'block', padding: 24 }}>{search ? '未找到匹配的服务器' : '暂无服务器'}</Text></Table.Cell></Table.Row>
                ) : filtered.map((client) => (
                  <SortableRow
                    key={client.uuid}
                    node={client}
      ...
      />
      <AddDialog open={addOpen} onOpenChange={setAddOpen} onSaved={loadClients} />
      <EditDialog client={editClient} open={editOpen} onOpenChange={setEditOpen} onSaved={loadClients} />
      <RotateTokenDialog client={rotateTokenClient} open={rotateTokenOpen} onOpenChange={setRotateTokenOpen} onRotated={loadClients} />
      <DeleteDialog client={deleteClient} open={deleteOpen} onOpenChange={setDeleteOpen} onDeleted={loadClients} />
